...
model{
      //Autopsy study likelihood
      for(i in 1:N_autopsy){
        if(epg_autopsy[i]==0 && worms_autopsy[i]==0)
          target += log(1);
        else
          target += poisson_lpmf(epg_autopsy[i] | epg_expected[worms_autopsy[i]]) + neg_binomial_2_lpmf(worms_autopsy[i] | M[N_studies], k[N_studies]);
        }
