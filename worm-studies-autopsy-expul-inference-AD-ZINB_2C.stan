...
transformed parameters{
   ...
   for(i in 1:max_worm){
     epg_expected_TH[i] = (i*L0[1]*M0[1])/(i+M0[1]); //algebraic decay function (THAILAND)
     epg_expected_LAO[i] = (i*L0[2]*M0[2])/(i+M0[2]); //algebraic decay function (LAOS)
     sens[i] = i/(sens_b+i);   
   }
