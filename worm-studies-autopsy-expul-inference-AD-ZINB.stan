...
parameters {
  ...
  real<lower=0, upper=1> sens; //sensitivity parameter (ZINB)
}
   ...
   for(i in 1:N_expul){
      if(epg_expul[i]==0 && worms_expul[i]==0){ //if no eggs or worms observed
        ...
        for(j in 2:delta_worm){ //false negative worms and eggs
            marginal_expul[i,j] = log_sum_exp(bernoulli_lpmf(0 | sens), bernoulli_lpmf(1 | sens) + neg_binomial_2_lpmf(0 | (epg_expected[(worms_expul[i]+j-1)]),h)) + neg_binomial_2_lpmf((j-1) | M[study_id[i]], k[study_id[i]]) + binomial_lpmf(0 | (j-1), pr_recovery);
          }
      }
      else if(epg_expul[i]==0 && worms_expul[i]>0){ //worms but no eggs (false negative eggs)
          for(j in 1:delta_worm){
            marginal_expul[i,j] = log_sum_exp(bernoulli_lpmf(0 | sens), bernoulli_lpmf(1 | sens) + neg_binomial_2_lpmf(0 | (epg_expected[(worms_expul[i]+j-1)]),h)) + neg_binomial_2_lpmf((worms_expul[i]+j-1) | M[study_id[i]], k[study_id[i]]) + binomial_lpmf(worms_expul[i] | (worms_expul[i]+j-1), pr_recovery);
          }
        }
           ...
           for(j in 2:delta_worm){
              marginal_expul[i,j] = bernoulli_lpmf(1 | sens) + neg_binomial_2_lpmf(epg_expul[i] | (epg_expected[(worms_expul[i]+j-1)]),h) + neg_binomial_2_lpmf((j-1) | M[study_id[i]], k[study_id[i]]) + binomial_lpmf(0 | (j-1), pr_recovery); 
           }
        }
        else{  //if >0 worms and eggs observed
          for(j in 1:delta_worm){
            marginal_expul[i,j] = bernoulli_lpmf(1 | sens) + neg_binomial_2_lpmf(epg_expul[i] | (epg_expected[(worms_expul[i]+j-1)]),h) + neg_binomial_2_lpmf((worms_expul[i]+j-1) | M[study_id[i]], k[study_id[i]]) + binomial_lpmf(worms_expul[i] | (worms_expul[i]+j-1), pr_recovery);
          }
        }
      }
    ...
    for(i in 1:N_autopsy){
        if(epg_autopsy[i]==0 && worms_autopsy[i]==0) //no eggs or worms
          ...
          for(j in 1:4){
            marginal_autopsy[i,j] = log_sum_exp(bernoulli_lpmf(0 | sens), bernoulli_lpmf(1 | sens) + neg_binomial_2_lpmf(epg_autopsy[i] | epg_expected[worms_autopsy[i]]*j,h)) + neg_binomial_2_lpmf(worms_autopsy[i] | M[N_studies], k[N_studies]);
          }
        }else{ //positive for eggs and worms
          for(j in 1:4){
            marginal_autopsy[i,j] = bernoulli_lpmf(1 | sens) + neg_binomial_2_lpmf(epg_autopsy[i] | epg_expected[worms_autopsy[i]]*j,h) + neg_binomial_2_lpmf(worms_autopsy[i] | M[N_studies], k[N_studies]);
          }
        }
    }
}
...
model{
    ...
    k_sd ~ normal(0.5, 1);
    h ~ normal(20, 2);
    sens ~ beta(8, 2); //Faecal egg count sensitivity
}
