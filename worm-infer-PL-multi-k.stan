...
parameters {
  ...
  real<lower=0> k[(N_clusters+N_studies)]; //worm dispersion parameter
  ...
  real<lower=0> k_mu; //Normal hyperprior of k
  real<lower=0> k_sd; //Normal hyperprior of k
...
transformed parameters {
  ...
  for(i in 1:(N_clusters+N_studies))
   p[i] = 1-(1+M[i]/k[i])^-k[i]; //calculate p for each cluster
}
...
model {
  ...
  for(i in 1:Nw){
    ...
    if (E[i]==0 && W[i]==0)
        ...
        target += poisson_lpmf(E[i] | expected_eggs[i]) + neg_binomial_2_lpmf(W[i] | M[(study[i]+N_clusters)], k[(study[i]+N_clusters)]); 
  }
  ...
  for(i in 1:N_clusters){
    ...
    for(j in 1:big_int){
      worm_prior[i,j] = neg_binomial_2_lpmf((j-1) | M[i], k[i]);
    }
  }
...
//Calculate probabilites
  ...
  M_beta ~ normal(0.5, 1);
  k ~ normal(k_mu, k_sd);   //strong prior on k - Burli et al.
  k_mu ~ normal(0.1, 0.1);
  k_sd ~ normal(0.1, 0.1);
